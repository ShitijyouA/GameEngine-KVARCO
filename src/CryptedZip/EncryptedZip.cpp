...
void EncryptedZip_impl<LoggerType>::AddPackFile(const fsys::path& source)
{
	...
	{
		parent_dir_=fsys::absolute(source.parent_path());
...
template<typename LoggerType>
void EncryptedZip_impl<LoggerType>::AddPackFile(const fsys::path& source,fsys::path& parent)
{
	if(!fsys::exists(source)) return;

	if(fsys::is_directory(parent)) parent_dir_=fsys::absolute(parent);
...
void EncryptedZip_impl<LoggerType>::InsertAllFilePath(const fsys::path& dir)
{
	...
	for(fsys::recursive_directory_iterator di(dir); di!=end; ++di)
	{
...
void EncryptedZip_impl<LoggerType>::MakeRltPathList(detail::PathList& rls_path_list)
{
	...
	for(detail::PathList::iterator i=input_file_path_list_.begin(); i!=input_file_path_list_.end(); ++i)
	{
		...
		fsys::path rlt_path;
#ifndef KVARCO_CRYPTED_ZIP_REMOVE_PARENT_DIR
		--ii;
#endif
		for(; ii!=i->end(); ++ii) rlt_path/=*ii;
...
void EncryptedZip_impl<LoggerType>::MakeCentralHeaderList()
{
	...
	)
	{
		...
		tmp.data_pos_	=0;
		tmp.raw_size_	=fsys::file_size(fsys::absolute(*ii,parent_dir_));
...
bool EncryptedZip_impl<LoggerType>::WriteToTemp(header::PathAndCentralHeader& it,DstStreamType& dst_stream)
{
	...
	//open new temp file
	fsys::path tmp_path=fsys::temp_directory_path()/fsys::path("tmp");
	std::ofstream fsink(tmp_path.string().c_str(),std::ios::out | std::ios::binary);
...
bool EncryptedZip_impl<LoggerType>::WriteToDst(header::PathAndCentralHeader& it,DstStreamType& dst_stream)
{
	//open temp file
	fsys::path tmp_path=fsys::temp_directory_path()/fsys::path("tmp");
	std::ifstream tmp_istream(tmp_path.string().c_str(),std::ios::in | std::ios::binary);
...
bool EncryptedZip_impl<LoggerType>::ZipToFile(const fsys::path& dst)
{
	...
	fsys::path tmp_path=fsys::temp_directory_path()/fsys::path("tmp");
	if(fsys::exists(tmp_path)) fsys::remove(tmp_path);
